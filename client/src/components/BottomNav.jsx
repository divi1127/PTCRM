...
import {
  LayoutDashboard, Target, Users, Calendar, MapPin, Trophy, FileSpreadsheet
} from 'lucide-react';
...
const BOTTOM_NAV = {
  admin: [
    ...
    { to: '/admin/import',    icon: MapPin,            label: 'Map' },
  ],
  employee: [
    ...
    { to: '/employee/map',       icon: MapPin,           label: 'Map' },
  ],
};
  ...
  return (
    <nav className="bottom-nav">
      {items.map(({ to, icon: Icon, label }) => {
        // Allow the tab to be highlighted when on a sub/view of the same page
        const isActive = location.pathname === to || (label === 'Map' && (location.pathname === '/admin/import' || location.pathname === '/employee/map'));
        return (
          <NavLink
            key={to}
            to={to}
